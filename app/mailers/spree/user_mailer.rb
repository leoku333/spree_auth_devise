module Spree
  class UserMailer < BaseMailer
    def reset_password_instructions(user, token, *args)
      @edit_password_reset_url = spree.edit_user_password_url(:reset_password_token => token, :host => Spree::Store.current.url)
    ...
    def confirmation_instructions(user, token, opts={})
      @confirmation_url = spree.user_confirmation_url(:confirmation_token => token, :host => Spree::Store.current.url)
